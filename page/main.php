    ...
    <div class="kiosk-index mt-5">
        ...
        <?php else : ?>
            <?php foreach ($kiosks as $index => $kiosk) : ?>
                ...
                ?>
                <div class="kiosk-container border-slate-900 p-1 w-full bg-rose-200 border rounded-lg mb-4">
...
<script>
    function payment(installmentNo, adminPhone, remainingAmount, kioskNumber) {
        ...
        Swal.fire({
            title: `ชำระเงินสำหรับตู้หมายเลข ${kioskNumber}`,
            html: `
    ...
    `,
        ...
        document.getElementById("slipForm").onsubmit = function(event) {
            ...
            if (allowedExtensions.includes(fileExtension)) {
                formData.append("files", slipFile); // ส่งไฟล์สลิป

                fetch('../api/upload_slip.php', {
                        method: 'POST',
                        body: formData
                    })
                    ...
                    .then(data => {
                        if (data.success) {
                            Swal.fire({
                                ...
                                updatePaymentStatus(installmentNo, kioskNumber);
                                location.reload();
                            });
                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: 'ล้มเหลว',
                                text: data.message || 'เกิดข้อผิดพลาดในการตรวจสอบสลิป',
                            });
                        }
                    })
                    .catch(error => {
                        ...
                        Swal.fire({
                            ...
                            text: 'ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์ได้ โปรดลองใหม่อีกครั้ง.',
                        });
                    });
            } else {
                Swal.fire({
                    ...
                    text: 'โปรดอัปโหลดไฟล์ในรูปแบบ JPG, JPEG, PNG, JFIF หรือ WEBP',
                });
            }
        };
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'สำเร็จ',
                        text: 'สถานะการชำระเงินได้รับการอัปเดตเรียบร้อยแล้ว',
                    });
                } else {
                    Swal.fire({
                        icon: 'error',
                        title: 'ล้มเหลว',
                        text: data.message || 'เกิดข้อผิดพลาดในการอัปเดตสถานะการชำระเงิน',
                    });
                }
            })
            .catch(error => {
